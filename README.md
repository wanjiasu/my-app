...
The project includes two hot reloads, one for the backend and one for the frontend, which automatically restart when they detect changes. This ensures that the application is always up-to-date without needing manual restarts.

- The **backend hot reload** monitors changes to the API code.
- The **frontend hot reload** monitors changes to the `openapi.json` schema generated by the backend.
